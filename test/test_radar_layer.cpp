...
TEST_CASE_METHOD(ROS2Fixture, "Costmap assertions", "[radar_layer]") {
  ...
  SECTION("Covariance scaling factor modifies how cost is spread around mean") {
   ...
   SECTION("ROSCON") {
    ...
    node->publishTf(0, 0, 0.7, node->get_clock()->now());
    ...
    node->set_parameter(rclcpp::Parameter("radar_layer.covariance_scaling_factor", 1.0));
    node->set_parameter(rclcpp::Parameter("radar_layer.number_of_time_steps", 1));
    node->set_parameter(rclcpp::Parameter("radar_layer.sample_time", 0.1));
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      node->publishMarkerArray(
        ...
        0.1, 0.1, 0.1,
        node->get_clock()->now());
      //node->set_parameter(rclcpp::Parameter("radar_layer.number_of_time_steps", std::min(100, i)));
      layers.updateMap(0, 0, 0);
      costmap_publisher_->publishCostmap();
      std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    }
